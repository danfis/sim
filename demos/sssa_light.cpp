...
#include "sssa_light.hpp"
#include <sim/msg.hpp>
#include <sim/common.hpp>

#include <sim/robot/meshes/sssa_body.hpp>
#include <sim/robot/meshes/sssa_arm.hpp>
#include <sim/robot/meshes/sssa_active_wheel.hpp>
#include <sim/robot/meshes/sssa_passive_wheel.hpp>
#include <sim/robot/meshes/sssa_passive_wheel90.hpp>
#include <list>
...
SSSALight::SSSALight(sim::World *w, const Vec3 &pos,
...
osg::Vec4 SSSALight::chasisColor() const
{
...
void SSSALight::setChasisColor(const osg::Vec4 &color)
{
...
Scalar SSSALight::armAngle() const
{
    return _arm.joint->angle();
}

void SSSALight::fixArm()
{
...
void SSSALight::unfixArm()
{
    _arm.joint->setParamLimitLoHi(-M_PI / 2., M_PI / 2.);
    _arm.fixed = false;
}

bool SSSALight::reachArmAngle(Scalar angle)
{
...
void SSSALight::setVelArm(sim::Scalar vel)
{
    _arm.vel = vel;
...
void SSSALight::addVelArm(sim::Scalar d)
{
    setVelArm(_arm.vel + d);
}

/*
void SSSALight::setVelLeft(sim::Scalar v)
{
    _wleft.vel = v;
...
void SSSALight::addVelLeft(sim::Scalar d)
{
    setVelLeft(_wleft.vel + d);
}

void SSSALight::setVelRight(sim::Scalar v)
{
    _wright.vel = v;
    for (size_t i = 0; i < 6; i++)
...
void SSSALight::addVelRight(sim::Scalar d)
{
    setVelRight(_wright.vel + d);
}
*/

void SSSALight::socketSetup(size_t idx, Vec3 *pos, Vec3 *dir, Vec3 *up) const
{
    if (idx >= 3)
        return;
...
void SSSALight::ballSetup(Vec3 *pos, Vec3 *dir, Vec3 *up) const
{
...
int SSSALight::canConnectTo(const SSSALight &robot) const
{
...
bool SSSALight::connectTo(SSSALight &robot)
{
    int socket_num;
    Vec3 pos, dir, up;
...
bool SSSALight::isConnected(const SSSALight *robot) const
{
    return _ball_conn == robot;
}

bool SSSALight::isSocketConnectedTo(size_t idx, const SSSALight *robot) const
{
    return _sock_conn[idx] == robot;
}

bool SSSALight::isAnySocketConnectedTo(const SSSALight *robot) const
{
    for (size_t i = 0; i < 3; i++){
...
void SSSALight::_createChasis(const osg::Vec4 &color)
{
    sim::Body *b;
    int id;
...
void SSSALight::_createArm(const osg::Vec4 &color)
{
    sim::Body *b;
...
void SSSALight::_createArmJoint()
{
}

/*
void SSSALight::_createWheels()
{
    sim::Joint *j;
