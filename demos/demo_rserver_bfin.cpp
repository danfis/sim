...
class SSSA : public sim::comp::SSSA {
  ...
  public:
    ...
    {
        _rf = new sim::sensor::RangeFinder(50, 181, M_PI);
...
class S : public sim::Sim {
  public:
    ...
    void createArena()
    {
        ...
        id = c->addBox(Vec3(0.1, 20., 0.5), SIM_BODY_DEFAULT_VIS, Vec3(-10, 0., .25));
        c->visBody(id)->setColor(color);
        c->visBody(id)->setTexture("wood.ppm");
        id = c->addBox(Vec3(0.1, 20., 0.5), SIM_BODY_DEFAULT_VIS, Vec3(10, 0., .25));
        c->visBody(id)->setColor(color);
        c->visBody(id)->setTexture("wood.ppm");
        id = c->addBox(Vec3(20., .1, 0.3), SIM_BODY_DEFAULT_VIS, Vec3(0., 10, .25));
        c->visBody(id)->setColor(color);
        c->visBody(id)->setTexture("wood.ppm");
        id = c->addBox(Vec3(20., .1, 0.3), SIM_BODY_DEFAULT_VIS, Vec3(0., -10, .25));
        c->visBody(id)->setColor(color);
        ...
        id = c->addCube(2, SIM_BODY_DEFAULT_VIS, Vec3(0., 0., 1.));
    ...
    void createRobot()
    {
        ...
        rob = new SSSA(10, Vec3(5., -2., .6), Quat(Vec3(0., 0., 1.), M_PI));
        addComponent(rob);


        /*
        rob = new SSSA(11, Vec3(.246, 2., .6));
        addComponent(rob);

        rob = new SSSA(12, Vec3(2., 4.254, .6), Quat(Vec3(0., 0., 1.), M_PI / 2.));
        addComponent(rob);

        rob = new SSSA(13, Vec3(2., 0.246, .6), Quat(Vec3(0., 0., 1.), -M_PI / 2.));
        addComponent(rob);
        */
