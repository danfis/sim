...
//! Constructor
Surf::Surf(IplImage *img, std::vector<sim::alg::SurfLandmark> &ipts)
: ipts(ipts)
{
  this->img = img;
}
...
{
  // Check there are sim::alg::SurfLandmarks to be described
  if (!ipts.size()) return;
  ...
  if (upright)
  {
    ...
    {
      // Set the sim::alg::SurfLandmark to be described
      index = i;
...
//! Assign the supplied sim::alg::SurfLandmark an orientation
void Surf::getOrientation()
{
  sim::alg::SurfLandmark *ipt = &ipts[index];
