...
void drawIpoints(IplImage *img, vector<sim::alg::SurfLandmark> &ipts, int tailSize)
{
    /*
...
void drawIpoint(IplImage *img, sim::alg::SurfLandmark &ipt, int tailSize)
{
    /*
  float s, o;
  int r1, c1, r2, c2, lap;
...
//! Draw a single feature on the image
void drawPoints(IplImage *img, vector<sim::alg::SurfLandmark> &ipts)
{
    /*
  float s, o;
  int r1, c1;
...
void drawWindows(IplImage *img, vector<sim::alg::SurfLandmark> &ipts)
{
  sim::alg::SurfLandmark *ipt;
  float s, o, cd, sd;
  int x, y;
...
void saveSurf(char *filename, vector<sim::alg::SurfLandmark> &ipts)
{
  ...
  for(unsigned int i=0; i < ipts.size(); i++) 
  {
    ...
    outfile << ipts.at(i).laplacian_sign << " ";
    outfile << ipts.at(i).scale << " ";
    for(int j=0; j<64; j++)
      outfile << ipts.at(i).desc[j] << " ";
...
void loadSurf(char *filename, vector<sim::alg::SurfLandmark> &ipts)
{
