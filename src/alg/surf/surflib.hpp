...
#ifndef _SIM_ALG_SURF_SURFLIB_HPP_
#define _SIM_ALG_SURF_SURFLIB_HPP_
...
inline void surfDetDes(IplImage *img,  /* image to find Ipoints in */
                       std::vector<sim::alg::SurfLandmark> &ipts, /* reference to vector of Ipoints */
                       bool upright = false, /* run in rotation invariant mode? */
                       int octaves = 5, /* number of octaves to calculate */
                       int intervals = 4, /* number of intervals per octave */
                       int init_sample = 2, /* initial sampling step */
                       float thres = 0.0004f /* blob response threshold */)
{
