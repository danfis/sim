...
//  - Create Kmeans object and call Run with std::vector<sim::alg::SurfLandmark>.
...
public:
  ...
  //! Do it all!
  void Run(std::vector<sim::alg::SurfLandmark> *ipts, int clusters, bool init = false);

  //! Set the ipts to be used
  void SetIpoints(std::vector<sim::alg::SurfLandmark> *ipts);

  //! Randomly distribute 'n' clusters
  void InitRandomClusters(int n);

  //! Assign sim::alg::SurfLandmarks to clusters
  ...
  float Distance(sim::alg::SurfLandmark &ip1, sim::alg::SurfLandmark &ip2);

  //! Vector stores ipoints for this run
  std::vector<sim::alg::SurfLandmark> *ipts;

  //! Vector stores cluster centers
  std::vector<sim::alg::SurfLandmark> clusters;

};

//-------------------------------------------------------

void Kmeans::Run(std::vector<sim::alg::SurfLandmark> *ipts, int clusters, bool init)
{
  if (!ipts->size()) return;
...
void Kmeans::SetIpoints(std::vector<sim::alg::SurfLandmark> *ipts)
{
  this->ipts = ipts;
}
...
bool Kmeans::AssignToClusters()
{
  ...
  // loop over all sim::alg::SurfLandmarks and assign each to closest cluster
...
void Kmeans::RepositionClusters()
{
  ...
  for (unsigned int i = 0; i < clusters.size(); ++i)
  {
    ...
    for (unsigned int j = 0; j < ipts->size(); ++j)
    {
      ...
      {
        sim::alg::SurfLandmark ip = ipts->at(j);
        x += ip.x;
        y += ip.y;
        dx += ip.dx;
        dy += ip.dy;
...
float Kmeans::Distance(sim::alg::SurfLandmark &ip1, sim::alg::SurfLandmark &ip2)
{
